...
def foresight_brief_md(fg) -> str:
    ...
    lines.append("_AI-surfaced candidate risks for an analyst to pressure-test — not "
                 "predictions. Each crosses this week's research signals with broader "
                 "societal forces._\n")
...
with tab_foresight:
    ...
    st.info(
        ...
        "not predictions.** Each one is generated by crossing *this week's* research "
        "signals (the digest below) with broader societal forces across a fixed scanning "
        "framework, to surface risks that sit in the **seam between domains** — the kind "
        "no single community is tracking. Claude is widening the aperture; **human "
        "judgment goes on top.** Treat every item as a hypothesis to investigate, and "
        "note each card's *Extrapolation* line for what goes beyond the data.",
        icon="🔮",
    )

    if not fg:
        st.warning(
            "**Foresight synthesis unavailable for this snapshot.** This pass uses the "
            "Claude API (like the rest of the analysis layer); it's skipped when no "
            "`ANTHROPIC_API_KEY` is configured for the weekly refresh. Add the secret and "
            "re-run the refresh to populate it.",
            icon="🔌",
        )
    else:
        look = _arxiv_lookup(snap)
        risks = fg.get("risks", [])
        st.markdown(f"**{len(risks)} candidate risk(s)** generated from this week's signals.")
        for i, r in enumerate(risks, 1):
            with st.container(border=True):
                ...
                # Link any cited source papers back to the real source.
...
Claude returns 2–4 candidate risks, each with a six-part structure (statement, derived-
